...
static uint8_t peakHoldY[128];  // peak hold Y positions (saves 128 bytes vs uint16_t rssi)
int vfo;
...
static void ResetBlacklist() {
  ...
  memset(peakHoldY, 0, sizeof(peakHoldY));
#ifdef ENABLE_SCAN_RANGES
...
static void DrawSpectrum() {
  for (uint8_t x = 0; x < 128; ++x) {
    uint8_t idx = x >> settings.stepsCount;
    uint16_t rssi = rssiHistory[idx];
    if (rssi != RSSI_MAX_VALUE) {
      uint8_t y = Rssi2Y(rssi);
      DrawVLine(y, DrawingEndY, x, true);

      // update peak hold (store minimum Y = highest signal)
      if (peakHoldY[idx] == 0 || y < peakHoldY[idx])
        peakHoldY[idx] = y;
    }

    // draw peak hold as dotted pixel
    if (peakHoldY[idx] > 0 && peakHoldY[idx] < DrawingEndY && (x & 1)) {
      PutPixel(x, peakHoldY[idx], true);
    }
  }
}
