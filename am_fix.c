...
//
// [IMPROVED] Better RSSI averaging, adaptive hysteresis and hold timing
//            to reduce gain hunting and improve AM audio stability
...
// [IMPROVED] 4-sample sliding window RSSI buffer for better noise/spike immunity
//            (original used only 2-sample average which was susceptible to spikes)
#define RSSI_BUF_SIZE 4
static int16_t rssi_buf[2][RSSI_BUF_SIZE];
static uint8_t rssi_buf_count[2] = {0, 0};  // how many valid samples we have

// holds the previous RSSI level (kept for compatibility with AM_fix_print_data)
int16_t prev_rssi[2] = {0, 0};
// to help reduce gain hunting, peak hold count down tick
unsigned int hold_counter[2] = {0, 0};

// [IMPROVED] gain increase skip counter - slows down gain recovery to avoid
//            pumping artifacts on AM audio (original increased every 10ms tick)
static uint8_t gain_increase_skip[2] = {0, 0};
...
{	// reset the AM fixer upper
	...
	// [IMPROVED] reset RSSI sliding window
	memset(rssi_buf[vfo], 0, sizeof(rssi_buf[vfo]));
	rssi_buf_count[vfo] = 0;
	gain_increase_skip[vfo] = 0;
}
...
void AM_fix_10ms(const unsigned vfo)
{
	...
	int16_t rssi;
	{
		// [IMPROVED] 4-sample sliding window average for better noise/spike immunity
		// Original code only averaged 2 samples (old + new), which was easily
		// fooled by a single RSSI spike causing unnecessary gain changes.
		// 4-sample window provides ~6dB better spike rejection while still
		// responding quickly enough to real signal level changes (40ms window).
		const int16_t new_rssi = BK4819_GetRSSI();

		// shift buffer: newest at [0], oldest falls off
		for (int i = RSSI_BUF_SIZE - 1; i > 0; i--)
			rssi_buf[vfo][i] = rssi_buf[vfo][i - 1];
		rssi_buf[vfo][0] = new_rssi;

		if (rssi_buf_count[vfo] < RSSI_BUF_SIZE)
			rssi_buf_count[vfo]++;

		// compute average over available samples
		int32_t sum = 0;
		for (int i = 0; i < rssi_buf_count[vfo]; i++)
			sum += rssi_buf[vfo][i];
		rssi = (int16_t)(sum / rssi_buf_count[vfo]);

		// keep prev_rssi updated for compatibility (AM_fix_print_data uses it)
	...
	if (diff_dB > 0) {	// decrease gain
		...
		if (gain_table_index[vfo] != index)
		{
			...
			// [IMPROVED] adaptive hold time based on how much gain we just dropped
			// Large gain drops (strong signal suddenly appeared) need shorter hold
			// so we can track rapid changes. Small adjustments use longer hold
			// to prevent gain hunting on fluctuating signals.
			// Original: fixed 300ms always
			const int gain_drop = (int)gain_table_index_prev[vfo] - (int)index;
			if (gain_drop > 5)
				hold_counter[vfo] = 20;       // 200ms - large drop, track quickly
			else
				hold_counter[vfo] = 40;       // 400ms - small drop, stay stable
		}

		// reset the gain increase skip counter when we're actively reducing
		gain_increase_skip[vfo] = 0;
	}

	// [IMPROVED] adaptive hysteresis based on signal proximity to saturation
	// When signal is well above desired level (strong signal), use tighter
	// hysteresis (-4dB) so we react faster to prevent saturation/distortion.
	// When signal is near or below desired level, use wider hysteresis (-8dB)
	// to prevent gain hunting on marginal/fluctuating signals.
	// Original: fixed -6dB always
	{
		const int16_t hysteresis = (diff_dB > 6) ? -4 : -8;
		if (diff_dB >= hysteresis)
			hold_counter[vfo] = MAX(hold_counter[vfo], 30u);  // at least 300ms hold
	}

	if (hold_counter[vfo] == 0)
	{	// hold has been released, we're free to increase gain

		// [IMPROVED] slow down gain increase by skipping every other tick
		// This prevents the "pumping" effect where gain rapidly increases
		// during brief pauses in AM audio (carrier still present but
		// modulation dip), then immediately has to decrease again when
		// audio returns. The original code increased gain every 10ms tick
		// which was too aggressive. Now we increase every 20ms (every other tick).
		if (gain_increase_skip[vfo] == 0) {
			gain_increase_skip[vfo] = 1;  // skip next tick
